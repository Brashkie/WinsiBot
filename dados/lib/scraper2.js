...
function wallpaper2(title) {
    return new Promise((resolve, reject) => {
        axios.get(`https://www.besthdwallpaper.com/search?CurrentPage=1&q=${title}`)
        .then(({ data }) => {
            ...
            $('div.grid-item').each(function (a, b) {
                hasil.push({
                   ...
                   chikititaaa: $(b).find('img').attr('data-src')
                })
            })
            resolve(hasil)
        })
...
module.exports = { pinterest2, wallpaper2, wikimedia2, porno, hentai2, quotesAnime,  gpwhatsapp, hentaistube, nerding, apkmodhacker, xvideos, uptodown, pornhub, soundl, st, gpsrc, dafontSearch, dafontDown, igstalk,  apkmody }