...
// ─── Nivel de cuenta (#settier) ───────────────────────────────────────────────
// Puramente una ETIQUETA por ahora — no cambia ningún beneficio de juego, solo
// se usa para categorizar en comandos como #bots. El campo `premium` (abajo)
// sigue siendo el que de verdad gatea beneficios existentes (tasa de éxito en
// #crime, montos de #slut, etc.) — asignar un tier no lo toca.
export type UserTier = 'free' | 'vip' | 'premium' | 'pro' | 'promax'
...
export interface UserData {
  ...
  tier?: UserTier // etiqueta de categoría (#settier) — ver comentario en UserTier arriba
  autoLevelUp: boolean
  // ── Anti-spam interno ──────────────────────────────────────────────────────
  spam: number
  lastSpam: number
...
export function expForLevel(level: number): number {
  return Math.floor(80 * (level + 1) ** 1.7)
}
