...
import { color, components, themes } from 'ansimax'
...
const NO_RETRY_CODES = new Set([DisconnectReason.loggedOut, 440, 401])
...
export class WinsiSocket extends EventEmitter3<WinsiEvents> {
  ...
  // ─── Pulso de presencia ────────────────────────────────────────────────────
  // markOnlineOnConnect=false (abajo) mantiene al bot sin mostrarse "en línea"
  // — pero el keepAliveIntervalMs de Baileys es solo un ping de transporte de
  // bajo nivel: confirma que el socket sigue abierto, no le dice a WhatsApp
  // "esta sesión está activa, seguí empujando mensajes en tiempo real". Se
  // sospecha que por eso el watchdog zombie de abajo se dispara cada
  // ~10-11 min TODO EL DÍA, no solo tras inactividad real — WhatsApp le baja
  // la prioridad de entrega en tiempo real a una sesión que nunca señala
  // presencia. Mandar 'available' y volver a 'unavailable' a los pocos
  // segundos, cada pocos minutos, es un pulso breve (no queda "en línea"
  // todo el tiempo) para probar esa hipótesis sin cambiar la privacidad del
  // número de forma permanente.
  private presencePulseTimer: ReturnType<typeof setInterval> | null = null
  private readonly PRESENCE_PULSE_INTERVAL_MS = 5 * 60_000
  private readonly PRESENCE_PULSE_DURATION_MS = 8_000
  ...
  private badMacByGroup = new Map<
    string,
    { count: number; windowStart: number; clearedAt: number }
  >()
  ...
  private cleanup() {
    ...
    if (this.sock) {
      ...
      try {
        this.sock.end(new Error(`${config.botName}: reconectando`))
      } catch (err) {
  ...
  private scheduleReconnect() {
    ...
    const delay =
      Math.min(1000 * 2 ** Math.min(this.retryCount, 6), 60_000) + Math.floor(Math.random() * 1000)
  ...
  async connect(): Promise<void> {
    ...
    const { state, saveCreds } = await useMultiFileAuthStateCBOR(config.sessionPath)
    ...
    this.sock.ev.on(
      ...
      ({ progress, isLatest, chats, messages, syncType }) => {
    ...
    const _ackBuffer: Array<{ id: string; status: number }> = []
    ...
    this.sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
      ...
      if (connection === 'open') {
        ...
        this._startPresencePulse()
        logger.info(`${config.botName} conectado`)
    ...
    this.sock.ev.on('messages.upsert', ({ messages, type }) => {
      ...
      for (const msg of messages) {
        ...
        if (msg.message.viewOnceMessageV2 || (msg.message as any).viewOnceMessageV2Extension) {
          handleViewOnce(this.sock!, msg.key.remoteJid ?? '', msg).catch((err) =>
            logger.warn({ err }, '[antiviewonce] handleViewOnce falló'),
          )
        }
  ...
  private _startZombieWatchdog(): void {
    ...
    this.zombieCheckTimer = setInterval(async () => {
      ...
      try {
        ...
        forceReconnect('Conexión zombie detectada (sin respuesta al ping) — forzando reconexión', {
          err,
        })
        return
      }
  ...
  // ─── Pulso de presencia ────────────────────────────────────────────────────
  // Ver comentario de PRESENCE_PULSE_INTERVAL_MS arriba. 'available' y a los
  // pocos segundos 'unavailable' — un pulso breve, no un "en línea" fijo.
  private _startPresencePulse(): void {
    if (this.presencePulseTimer) clearInterval(this.presencePulseTimer)

    this.presencePulseTimer = setInterval(async () => {
      if (!this.sock || this.isReconnecting) return
      try {
        await this.sock.sendPresenceUpdate('available')
        setTimeout(() => {
          this.sock?.sendPresenceUpdate('unavailable').catch(() => {})
        }, this.PRESENCE_PULSE_DURATION_MS).unref()
      } catch (err) {
        logger.debug({ err }, 'Pulso de presencia falló — no crítico')
      }
    }, this.PRESENCE_PULSE_INTERVAL_MS).unref()
  }
  ...
  private _doClearGlobal(count: number): void {
    ...
    import('@lib/session.js')
      .then(({ sessionClient }) => sessionClient.clearSignalSessions())
      .then((res) => {
        logger.info(
          `[socket] ${res.deleted} archivos Signal eliminados (clear global) — reconectando`,
        )
        this.requestReconnect()
      })
      .catch((err) => {
  ...
  private _doClearForGroup(groupJid: string, count: number, source: string): void {
    ...
    import('@lib/session.js')
      .then(({ sessionClient }) => sessionClient.clearSignalSessions(groupJid))
      .then((res) => {
        logger.info(
          `[socket] ${res.deleted} archivos Signal eliminados (scope: grupo) — reconectando`,
        )
