...
const command: Command = {
  ...
  async execute({ sock, jid, msg, sender, pushName, args, prefix }) {
    if (isOnCooldown(sender, 'lastRoulette', CD)) {
      const left = getCooldownLeft(sender, 'lastRoulette', CD)
      await safeSend(() =>
        sock.sendMessage(
          jid,
          {
    ...
    // Aplicar
    setCooldown(sender, 'lastRoulette')
    ...
    const betLabel =
      betType === 'color' ? `${COLOR_EMOJI[betColor!]} ${betColor}` : `🔢 número ${betNumber}`
