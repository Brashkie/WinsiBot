...
const CDS: Array<{ label: string; key: Parameters<typeof isOnCooldown>[1]; ms: number | null }> = [
  ...
  { label: 'Slut', key: 'lastHunt' as const, ms: 10 * 60_000 },
  { label: 'Crime', key: 'lastCrime' as const, ms: 15 * 60_000 },
  ...
  { label: 'Invertir', key: 'lastBet' as const, ms: 10_000 },
  { label: 'Roulette', key: 'lastRoulette' as const, ms: 15_000 },
]
...
const command: Command = {
  ...
  async execute({ sock, jid, msg, sender, pushName }) {
    ...
    const rows = CDS.map(({ label, key, ms }) => {
      const left =
        ms === null
          ? isOnCooldownDaily(sender, key)
            ? getDailyCooldownLeft()
            : 0
          : isOnCooldown(sender, key, ms)
            ? getCooldownLeft(sender, key, ms)
            : 0
